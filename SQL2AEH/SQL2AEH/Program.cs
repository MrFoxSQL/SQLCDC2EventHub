            ...
            do
            {
                ...
                if (resultCollection.Any())
                {
                    ...
                    foreach (var resultGroup in orderedByColumnName.Split(SQLBatchSize))
                    {
                        ...
                        updateOffsetQuery = "update dbo.SQL2AEH_TableOffset set LastMaxVal = convert(binary(10), '" + nextOffset + "', 1), LastUpdateDateTime = getdate() where TableName = '" + dataTableName + "'";
                        queryPerformer.PerformQuery(updateOffsetQuery);
                    }
                }

                // UPDATE OFFSET LAST CHECKED DATE
                updateOffsetQuery = "update dbo.SQL2AEH_TableOffset set LastCheckedDateTime = getdate() where TableName = '" + dataTableName + "'";
                queryPerformer.PerformQuery(updateOffsetQuery);
